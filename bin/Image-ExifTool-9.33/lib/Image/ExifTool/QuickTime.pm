...
%Image::ExifTool::QuickTime::Main = (
    ...
    udta => {
        Name => 'TopUserData',
        SubDirectory => { TagTable => 'Image::ExifTool::QuickTime::TopUserData' },
    },
);
...
%Image::ExifTool::QuickTime::TopUserData = (
    PROCESS_PROC => \&ProcessMOV,
    GROUPS => { 1 => 'FLIR', 2 => 'Camera' },
    NOTES => 'Tags written in MP4 videos from some FLIR cameras.',
    uuid => [
        {
            Name => 'FLIR_Parts',
            ...
            if ($tagInfo) {
                ...
                if ($subdir) {
                    ...
                    if (defined $key and (not $format or $format =~ /^string/) and
                        ...
                        not $$tagInfo{Binary} and defined $$exifTool{VALUE}{$key} and not ref $val)
                    {
