...
sub SetNewValue($;$$%)
{
    ...
    # remove leading family number if specified
    $family = $1 if $wantGroup and $wantGroup =~ s/^(\d+)//;
...
# handle group delete
#
    while ($tag eq '*' and not defined $value) {
...
#
    ...
    foreach $tagInfo (@matchingTags) {
        ...
        if ($wantGroup) {
            ...
            my ($grp1, $grp2);
            ($writeGroup, $grp1, $grp2) = $self->GetGroup($tagInfo);
            unless (lc($writeGroup) eq $lcWant or ($grp2 and lc($grp2) eq $lcWant)) {
                ...
                } elsif ($writeGroup eq 'QuickTime') {
                    next unless $movGroup and $grp1 and $grp1 eq 'Track#';
                    $writeGroup = $movGroup;
